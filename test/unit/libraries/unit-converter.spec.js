...
describe('unit-converter', () => {
  ...
  describe('.bytesReadableOrDefault', () => {
    it('returns readable value', () => {
      expect(bytesReadableOrDefault(10000)).to.eql({
        value: '9.77',
        units: 'KB'
      })
    })
    ...
    it('returns default value when parsing fails', () => {
      expect(bytesReadableOrDefault('a')).to.eql({
        value: '-',
        units: 'KB'
      })
    })
  ...
  describe('.timeDisplayOrDefault', () => {
    it('returns display value', () => {
      expect(timeDisplayOrDefault(60 * 60 + 60 + 1)).to.be.eql('01:01:01')
    })

    it('returns default value when parsing fails', () => {
      expect(timeDisplayOrDefault('a')).to.eql('--:--:--')
    })
  })
})
